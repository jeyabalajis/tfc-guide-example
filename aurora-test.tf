locals {
  ...
  tags = {
    ...
    ApplicationName = "casc-cn-aurora-postgres-dev"
    Environment     = "dev"
    Terraform       = "true"
  }
}
...
module "rds-aurora" {
  ...
  db_cluster_parameter_group_name = aws_db_parameter_group.db_param_group.id
  db_parameter_group_name         = aws_rds_cluster_parameter_group.db_cluster_param_group.id
